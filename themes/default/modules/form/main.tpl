      ...
      setTimeout(() => {
        ...
        for (const index in method) {
          if (method.hasOwnProperty(index)) {
            ...
            if (element.search(key) >= 0) {
              html += '<div class="suggest_item" onclick="selectMethod(\'' + name + '\', \'' + method[index]['name'] + '\')"><p class="right-click">' + method[index]['name'] + '<br>' + method[index]['symbol'] + '</p><button class="close right" data-dismiss="modal" onclick="removeMethod('+method[index]['id']+', \''+name+'\')">&times;</button></div>'
            }
          }
        }
        suggest.html(html)
      ...
      case 4:
        ...
        if (sampleCode) {         
          data = {
            ...
            status: getCheckbox('status'),
            samplecode: formInsertSampleCode.val(),
            sender: formInsertSenderUnit.val(),
            address: formInsertAddress.val(),
            xphone: formInsertXphone.val(),
      ...
      (response, status) => {
        checkResult(response, status).then(data => {
          ...
          if (data['form']['printer'] >= 1) {
            ...
            formInsertNumber2.val(data['form']['number'])
          ...
          if (data['form']['printer'] >= 2) {
            ...
            $(".status-" + data['form']['xstatus']).prop('checked', true)
          ...
          if (data['form']['printer'] >= 3) {
            ...
            formInsertXnote.val(data['xnote'])
      ...
      if (Object.keys(data).length) {
        ...
        switch (id) {
          ...
          case 2:
            ...
            html = html.replace('(xphone)', data['xphone'])
