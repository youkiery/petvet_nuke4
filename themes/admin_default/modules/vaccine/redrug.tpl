      ...
      <div class="modal-body">
        <form onsubmit="return edit_submit(event)">
          <div class="form-group">
            ...
            <input type="text" class="form-control" id="edit-drug">
          </div>
          <div class="form-group">
            <label> {lang.drugcome} </label>
            <input type="text" class="form-control" id="edit-cometime" readonly>
          </div>
          <div class="form-group">
            <label> {lang.drugcall} </label>
            <input type="text" class="form-control" id="edit-calltime" readonly>
          </div>
          <div class="form-group">
...
<div id="modal-remove" class="modal fade" role="dialog">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal">&times;</button>
        <span> {lang.redrug_remove} </span>
      </div>
      <div class="modal-footer">
        <button class="btn btn-info" onclick="remove_submit()">
          {lang.remove}
        </button>
        <button class="btn btn-info" data-dismiss="modal">
          {lang.cancel}
...
<script>
  ...
  $("#drugcome, #drugcall, #confirm_recall, #edit-cometime, #edit-calltime").datepicker({
   	format: 'dd/mm/yyyy',
    changeMonth: true,
    changeYear: true  
	});
  ...
  function insert(e) {
    ...
    if (customer > 0) {
      $.post(
        strHref,
        {action: "insert", note: $("#c_note").val(), customer: customer, drug: $("#drug").val(), drugcome: $("#drugcome").val(), drugcall: $("#drugcall").val(), filter: g_filter, page: page},
        (response, status) => {
          var data = JSON.parse(response);
          if (data["status"]) {
            $("#content").html(data["list"])
  ...
  function edit(id) {
    g_id = id
    $("#modal-edit-save").attr("disabled", true)
    $.post(
      strHref,
      {action: "getedit", id: id},
      (response, status) => {
        var data = JSON.parse(response) 
        if (data["status"]) {
          $("#modal-edit-save").attr("disabled", false)
          $("#modal-edit").modal("toggle")
          $("#edit-drug").html(data["drug"])
          $("#edit-cometime").val(data["cometime"])
          $("#edit-calltime").val(data["calltime"])
  ...
  function edit_submit(e) {
    ...
    if (g_id) {
      $.post(
        strHref,
        {action: "edit", id: id, filter: g_filter, page: page},
        (response, status) => {
          var data = JSON.parse(response) 
          if (data["status"]) {
            $("#content").html(data["list"])
  ...
  function remove_submit() {
    if (g_id) {
      $.post(
        strHref,
        {action: "remove", id: g_id, filter: g_filter, page: page},
        (response, status) => {
          var data = JSON.parse(response) 
          if (data["status"]) {
            $("#modal-remove").modal("toggle")
