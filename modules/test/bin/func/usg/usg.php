...
if (!empty($action)) {
	...
	switch ($action) {
		...
		case 'insert-pet':
			...
			$sql = "select * from `" . VAC_PREFIX . "_pet` where name = '$data[name]' and customerid = $data[id]";
			$query = $db->query($sql);
			if (!$query->rowCount()) {
				$sql = "insert into `" . VAC_PREFIX . "_pet` (name, customerid) values ('$data[name]', $data[id]);";
				
				$query = $db->query($sql);
				$id = $db->lastInsertId();
				if ($id) {
					$result["status"] = 1;
					$result["data"] = array("id" => $id, "name" => $data['name'], "customerid" => $data['id']);
				}
			}
			else {
				$result["status"] = 2;
