		...
		case 'doctoradd':
			...
			if (!empty($doctor)) {
				...
				if (!$result->fetch()) {
					...
					if ($db->query($sql)) {
						$ret["status"] = 1;
						$ret["data"] = doctorlist(NV_ROOTDIR . "/themes/" . $global_config['admin_theme'] . "/modules/" . $module_file, $lang_module);
					}
					else {
						$ret["status"] = 2;
					}
...
$xtpl = new XTemplate("doctor.tpl", NV_ROOTDIR . "/themes/" . $global_config['admin_theme'] . "/modules/" . $module_file);
$xtpl->assign("lang", $lang_module);

$xtpl->assign("list", doctorlist(NV_ROOTDIR . "/themes/" . $global_config['admin_theme'] . "/modules/" . $module_file, $lang_module));
