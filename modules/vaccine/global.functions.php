...
function user_usg() {
  ...
  $sql = "select a.id, a.cometime, a.calltime, a.status, a.image, a.note, a.birthday, a.birth, a.expectbirth, a.doctorid, b.id as petid, b.name as petname, c.name as customer, c.phone from `" . VAC_PREFIX . "_usg` a inner join `" . VAC_PREFIX . "_pet` b on a.$type between $from and $end and a.petid = b.id inner join `" . VAC_PREFIX . "_customer` c on b.customerid = c.id $where order by a.$type";
	$query = $db->query($sql);
	$list = array();
...
function user_vaccine() {
  ...
  switch ($page) {
    case 'today':
      $end = $today + 60 * 60 * 24;
      $where = "where ctime between $today and $end and status = $id";
    break;
    case 'retoday':
      $end = $today + 60 * 60 * 24;
      $where = "where calltime between $today and $end and status = $id";
    break;
    case 'search-all':
      $where = '';
    break;
    default:
      ...
      $from = $today - $time;
      $end = $today + $time;
      $where = "where calltime between $from and $end and status = $id";
  }
