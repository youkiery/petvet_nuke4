...
<script>
    var global = {
        ...
        selected: [],
        process: 0,
        allow: ['SHOP', 'SHOP>>Balo, giỏ xách', 'SHOP>>Bình xịt', 'SHOP>>Cát vệ sinh', 'SHOP>>Dầu tắm', 'SHOP>>Khay vệ sinh', 'SHOP>>Nhà, chuồng', 'SHOP>>Thuốc bán', 'SHOP>>Thuốc bán>>thuốc sát trung', 'SHOP>>Tô - chén', 'SHOP>>Vòng-cổ-khớp', 'SHOP>>Xích-dắt-yếm']
    ...
    //     for (const key in global['data']) {
    //         if (global['data'].hasOwnProperty(key)) {
    //             const element = global['data'][key];
    //             // key = Object.keys(global['data'])[0]
    //             // element = global['data'][key]
    //         }
    //     }
    //     global['data'] = {}
    //     notify('Đã lưu')
    ...
    // }
    // if (file.attachEvent) file.attachEvent('onchange', handler);
    // else file.addEventListener('change', handler, false);
    // if (file2.attachEvent) file2.attachEvent('onchange', handler);
    // else file2.addEventListener('change', handler, false);
