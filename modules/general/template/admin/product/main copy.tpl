...
<div class="form-group row">
    ...
    <button class="btn btn-info" onclick="$('#allower-modal').modal('show')">
        Danh sách quản lý
    </button>
...
<script>
    ...
    function filterUser() {
        vhttp.checkelse('', { action: 'filter-user', name: $("#allower-name").val() }).then(data => {
            $("#allower-insert-content").html(data['html'])
        })
    }

    function removeAllower(id) {
        vhttp.checkelse('', { action: 'remove-allower', id: id, name: $("#allower-name").val() }).then(data => {
            $("#allower-content").html(data['html'])
            $("#allower-insert-content").html(data['html2'])
        })
    }

    function insertAllower(id) {
        vhttp.checkelse('', { action: 'insert-allower', id: id, name: $("#allower-name").val() }).then(data => {
            $("#allower-content").html(data['html'])
            $("#allower-insert-content").html(data['html2'])
        })
    }
